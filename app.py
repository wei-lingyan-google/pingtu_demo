...
# ===================== 核心：读取iframe localStorage中的成绩 =====================
# 用JS读取localStorage中的成绩数据
score_data = st.js_execute("""
const data = localStorage.getItem('puzzle_score');
if (data) {
    localStorage.removeItem('puzzle_score'); // 读取后清除，避免重复提交
...
# 处理成绩数据
if score_data:
    time_sec = score_data.get("time", 0)
    step = score_data.get("step", 0)
...
# 清空排行榜逻辑（改用localStorage）
clear_data = st.js_execute("""
const clear = localStorage.getItem('clear_rank');
if (clear) {
    localStorage.removeItem('clear_rank');
    return true;
}
return false;
""")
...
// ===================== 关键修复：用localStorage传递数据，不跳URL =====================
function submitScore() {
    if(elapsedTime<=0||moves<=0) return;
    alert("你真是个棒人！！");
    // 把成绩数据存到localStorage，父页面会自动读取
    localStorage.setItem('puzzle_score', JSON.stringify({
        time: elapsedTime,
        step: moves
    }));
    // 不跳URL，页面会自动刷新
...
function clearRank() {
    localStorage.setItem('clear_rank', 'true');
}
